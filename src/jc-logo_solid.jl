...
function drawlogo(logo_width::Int)
    ...
    link_large_length = div(ref_dist, 5)
    link_large_width = div(ref_dist, 7)
...
for logo_width in (1024, 512, 128, 32) # large, medium, small and favicon sizes
    Drawing(logo_width, logo_width, joinpath("logo_images", "jc-logo_solid_"*string(logo_width)*"x"*string(logo_width)*".png"))
    drawlogo(logo_width)
    finish()
end
